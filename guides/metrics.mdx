---
title: "Metrics"
description: "Learn how to create and publish your metrics"
icon: chart-simple
...
Metrics are used to summarize dimensions or, sometimes, other metrics.

## Adding metrics to your model
...
| [description](#description)                         | No       | string                         | Description of the metric. A default description is created if this isn't included                                                                                                                            |
...
You can use the `format` parameter to have your metrics show in a particular format. Turntable supports spreadsheet-style format expressions for all metric types.
...
<Accordion title="(Legacy) format, compact, and round options">

Spreadsheet-style format expressions are the recommended way of adding formatting to your metrics in Turntable. There are legacy formatting options, listed below, which are less flexible than the spreadsheet-style formatting. 

<Info>

If you use both legacy and spreadsheet-style formatting options for a single metric, Turntable will ignore the legacy `format`, `compact`, and `round` options and only apply the spreadsheet-style formatting expression.
...
Filters are applied to metrics any time that metric is used in Turntable. Filters can only be used with [aggregate metric types](#metric-categories).
