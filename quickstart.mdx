...
```bash
poetry init -n --python ">=3.10, <3.12" --dependency vinyl --dev-dependency ipykernel && poetry install && poetry shell
```

<Note> 
If you have an existing poetry project, run `poetry add vinyl && poetry add -G dev ipykernel` and make sure you're in a poetry shell for the next step.
</Note>

<Info> For now, Vinyl projects must also be poetry projects. We're working on adding support for other package managers.</Info>
