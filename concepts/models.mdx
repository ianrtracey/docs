...
def top_shops(shops, txns):
    ...
    st.sort(by=[st.num_transactions.desc()])
...
stores_txns = stores.join(StoreNumTransactions(), ["store_nbr"])
...
@model(deps=[FlowerShops, Transactions, MetricStore])
def shop_sales(shops, txns, metric_store):
    st = shops.join(txns, ["store_nbr"])
    metric_store.metric(
        tbl=st,
        by=[st.store_nbr, st.city],
...
sales_metric = metric_store.select([
    metric_store.ts.truncate('M'),
    metric_store.store_nbr,
    metric_store.city,
    metric_store.sales
], trailing=[None, 3])
...
```python
@model(deps=[MetricStore])
def shop_sales_last_30_days(metric_store):
    sales_metric = metric_store.select([
        metric_store.ts.truncate('D'),
        metric_store.store_nbr,
        metric_store.city,
        metric_store.sales
    ], trailing=[None, 30])
